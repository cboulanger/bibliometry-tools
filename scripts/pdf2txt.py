...
Copied from https://github.com/pdfminer/pdfminer.six/blob/86e34873e4daeb7329aa7420208e82f8d41e38b6/tools/pdf2txt.py
"""
import argparse
import logging
import os
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="+",
        help="One or more paths to PDF files, or a directory, in which case all PDFs in the directory will be extracted to the same directory or to the directory specified in --output-dir.",
    )
    ...
    output_params.add_argument(
        ...
        "images are not extracted. If used with an input directory as the positional argument, the directory to store all files in (no image extraction)",
    )
    output_params.add_argument(
        "--layoutmode",
        "-Y",
...
def extract_dir(**kwargs):
    input_dir = kwargs['files'][0]
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    if os.path.isdir(parsed_args.files[0]):
        extract_dir(**vars(parsed_args))
    else:
