...
export default async function neo4j_export() {
   ...
   */
   async function couchbase_execute(query: string) : Promise<any[]> {
    let rows;
    let attempts = 3;
    let a = attempts;
    while (a > 0) {
   ...
   */
  async function neo4j_execute(statement: string, params: {[key:string]:any}={}): Promise<void> {
   ...
   * Publication nodes
   */
  async function mergePublicationNode(item: any, indent=0): Promise<void> {
    // create/update
